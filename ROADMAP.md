...
- [ ] `ImportJob` 最小模型（`id` / `source_path` / `status` / `progress` / `error`；不做分布式任务队列）
- [ ] 导入元数据 `SourceInfo`（`source` + `export_label` + `importer_version`，写入 `imports` 或 `ImportJob`）
